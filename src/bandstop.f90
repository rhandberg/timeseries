!****************************************************
!   BANDSTOP
!   Bandstop filtering using Weighted Least Squares
!
!   Input:
!       bandstop [options] [inputfile] [outputfile]
!
!   Options:
...
program bandsto
    ...
    open(50, file=inputfile, status='old', action='read')
        ...
        ! Allocate the data:d
    	allocate(t(1:N), f(1:N), w(1:N), bs(1:N), stat=err)
	...
	! Run the bandstop-filter:	
	bs = bandstop(t, f, w, nu1, nu2, nu_min, nu_max, dF)
	...
	do j = 1,N
		write(51, '(3ES26.16E3)') t(j), bs(j), 1.0_dbl/sqrt(w(j))
	enddo
	close(51)
